...
Analyze the uploaded dataset and automatically generate a business-focused Knowledge Graph based on the actual sample values provided below.
...
Instructions:
1. Identify all actual business entities (e.g., Customer, Dealer, Product, Material, Region, Invoice, Date, etc.) from the sample data and create them as graph nodes.
2. Identify relationships between these specific entities and create graph edges.
3. Numerical measures (Sales Quantity, Invoice Value, Revenue, Discount, GST, Profit, etc.) should be stored as node or edge properties, NOT as separate nodes.
4. Avoid technical database concepts (table names, column names, row counts, data types).
5. Focus on relationships that support analytics (e.g., High Selling Products, Top Customers, Seasonal Trends, Anomalous Transactions).

Output EXACTLY this JSON format - no markdown, no extra text:
{{
  "nodes": [
    {{"id": "p1", "label": "Product: Tyre", "type": "Product", "properties": {{"Price": 5000}}}},
    {{"id": "r1", "label": "Region: Central", "type": "Region", "properties": {{}}}}
  ],
  "edges": [
    {{"from": "p1", "to": "r1", "label": "HIGH_DEMAND_IN", "properties": {{"Quantity": 50}}}}
  ],
  "identified_node_types": ["Product", "Region", "Customer"],
  "identified_relationship_types": ["HIGH_DEMAND_IN", "PURCHASED", "SOLD_IN"],
  "graph_schema": ["(Product)-[:HIGH_DEMAND_IN]->(Region)"],
  "sample_cypher_queries": ["MATCH (p:Product)-[r:HIGH_DEMAND_IN]->(rg:Region) RETURN p, r, rg"],
  "business_insights": ["Tyre has a high demand in the Central region."],
  "suggested_graphrag_paths": ["Start from Product, follow HIGH_DEMAND_IN to Region"]
}}

Rules:
- The `id` of each node must be a unique string.
- The `from` and `to` in edges must refer to the exact `id` of nodes you created in the `nodes` array.
- Extract actual values from the sample data for labels (e.g. "Customer: John").
- DO NOT invent data. Only use entities found in the DB Tables and Sample Data.
"""
